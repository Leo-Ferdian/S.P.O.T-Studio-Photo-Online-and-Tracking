...
const packageAllowsAddons = computed(
  () => summary.value?.package?.price_type === 'per_package'
)
...
const confirmBooking = async () => {
  ...
  try {
    ...
    // Perbaikan alur: langsung ke pembayaran
    router.push('/booking/payment')
  } catch (error) {
    errorMessage.value =
      error.message || 'Gagal mengkonfirmasi pesanan.'
  } finally {
...
const formattedDateTime = computed(() => {
  if (!summary.value?.dateTime) return 'Waktu belum dipilih'
...
onMounted(() => {
  ...
  if (authStore.user) {
    customerName.value =
      authStore.user.full_name || bookingStore.customerDetails.name
    customerEmail.value =
      authStore.user.email || bookingStore.customerDetails.email
    customerWhatsapp.value =
      authStore.user.phone_number || bookingStore.customerDetails.whatsapp
  }
})
</script>
          ...
          <div class="bg-white border-3 border-outline p-4 flex justify-between items-start">
            <div>
              ...
              <p class="font-bold text-lg mt-2">
                {{ formatCurrency(summary.package.price) }}
              </p>
            </div>
        ...
        <form @submit.prevent="confirmBooking" class="space-y-4">
          ...
          <div v-if="packageAllowsAddons" class="pt-4">
            ...
            <p class="text-xs text-gray-600 mb-2">
              Harga paket sudah termasuk {{ summary.package.capacity }} orang.
            </p>
            ...
            <div
              ...
              <div>
                ...
                <p class="text-xs">
                  {{ formatCurrency(bookingStore.addOnPricePerPerson) }} / orang
                </p>
              </div>
