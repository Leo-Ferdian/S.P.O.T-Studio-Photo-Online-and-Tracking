<script setup>
    ...
    onMounted(() => {
        fetchBranches();
        // Beri sedikit delay agar DOM sempat diperbarui sebelum merender ikon
    ...
    <div class="bg-background min-h-screen font-display text-text-default pt-24 pb-12">
        <main class="container mx-auto px-4">
            ...
            <div v-else class="space-y-12 flex flex-col items-center">
                <!-- Menggunakan <router-link> untuk membuat seluruh kartu bisa diklik -->
                <router-link v-for="branch in branches" :key="branch.id" :to="`/location/${branch.slug}`"
                    class="max-w-md w-full group block">
                    <div class="border-4 border-outline group-hover:opacity-80 transition-opacity">
