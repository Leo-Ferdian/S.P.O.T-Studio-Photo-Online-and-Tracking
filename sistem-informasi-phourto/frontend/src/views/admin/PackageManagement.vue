...
// --- State Data Global ---
const branches = ref([]);
const rooms = ref([]); // State untuk menampung ruangan
const packages = ref([]);
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (currentView.value === 'add') {
            await apiClient.post('/admin/packages', { packageData, addons: [] });
            successMessage.value = 'Data paket baru berhasil disimpan!';
        } else {
            await apiClient.put(`/admin/packages/${form.value.id}`, { packageData, addons: [] });
        ...
        errorMessage.value = error.response?.data?.message || "Gagal menyimpan data paket.";
