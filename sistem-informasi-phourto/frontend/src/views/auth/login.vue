...
const handleLogin = async () => {
    ...
    try {
        // Panggil login (mengirim 2 argumen)
        await authStore.login(email.value, password.value);

        // Cegah admin login lewat form ini
        if (authStore.isAdmin) {
            authStore.logout();
            errorMessage.value = 'Akun Admin harus login via /admin/login.';
        }
    } catch (error) {
        errorMessage.value = error.message || 'Email atau password yang Anda masukkan salah.';
    } finally {
...
<template>
    <div class="bg-background min-h-screen flex flex-col items-center justify-center text-text-default relative">
        <div class="max-w-lg w-full p-8 space-y-6">
            ...
            <form @submit.prevent="handleLogin" class="space-y-4">
                ...
                <div>
                    ...
                    <input v-model="email" type="email" id="email" required
                        class="mt-1 w-full p-3 bg-primary text-text-inverse border-3 border-outline shadow-solid placeholder-gray-300
                        focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-background focus:ring-primary transition-all" />
                </div>
                ...
                <div>
                    ...
                    <div class="relative mt-1">
                        <input v-model="password" :type="isPasswordVisible ? 'text' : 'password'" id="password" required
                            class="w-full p-3 bg-primary text-text-inverse border-3 border-outline shadow-solid placeholder-gray-300
                            focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-background focus:ring-primary transition-all" />
                ...
                <button type="submit" :disabled="isLoading" class="w-full bg-primary text-text-default font-bold py-3 border-3 border-outline shadow-solid
                    hover:bg-red-600 active:shadow-none active:translate-x-1 active:translate-y-1 transition-all duration-100
            ...
            <!-- Tombol Login Google -->
            <button :disabled="isLoading" class="w-full bg-white text-text-default font-bold py-3 border-3 border-outline shadow-solid
                hover:bg-gray-200 active:shadow-none active:translate-x-1 active:translate-y-1 transition-all duration-100
