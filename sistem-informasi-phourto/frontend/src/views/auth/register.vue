<script setup>
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            // Pengalihan ke halaman login sudah ditangani di dalam auth.stores.js
        } catch (error) {
            errorMessage.value = error.response?.data?.message || 'Gagal mendaftar. Pastikan email belum terdaftar.';
        }
    };
</script>
    ...
    <div class="bg-background min-h-screen flex items-center justify-center font-display text-text-default">
        <div class="max-w-sm w-full p-8 space-y-6">
            ...
            <form @submit.prevent="handleRegister" class="space-y-4">
                ...
                <p class="text-xs text-center !mt-6"> <!-- PERBAIKAN DI SINI: !mt-6 untuk memaksa margin atas -->
        ...
        <div
            ...
            Already have an account? <router-link to="/login" class="hover:underline">Login</router-link>
        </div>
    </div>
</template>